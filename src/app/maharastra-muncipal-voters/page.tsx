  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      if (file.type === 'application/pdf') {
        ...
        const maxPages = Math.min(Math.max(1, pageLimit || 1), pdf.numPages);
        ...
        const LIMIT = Math.max(1, Math.min(concurrency || 1, 12));
        ...
        for (let start = 0; start < indices.length; start += LIMIT) {
          const slice = indices.slice(start, start + LIMIT);
          const batchResults = await Promise.all(slice.map(renderAndAnalyze));
          for (const voters of batchResults) {
                        ...
                        <div className="flex flex-col items-center gap-4 mt-6">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 w-full max-w-xl">
                              <div className="flex flex-col w-full">
                                <label className="text-xs text-muted-foreground mb-1">Max pages</label>
                                <Input type="number" min={1} max={5000} value={pageLimit}
                                  onChange={(e) => setPageLimit(Math.max(1, Math.min(5000, Number(e.target.value) || 1)))} />
                              </div>
                              <div className="flex flex-col w-full">
                                <label className="text-xs text-muted-foreground mb-1">Concurrency</label>
                                <Input type="number" min={1} max={12} value={concurrency}
                                  onChange={(e) => setConcurrency(Math.max(1, Math.min(12, Number(e.target.value) || 1)))} />
                              </div>
