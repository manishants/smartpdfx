...
export async function GET(req: Request) {
  ...
  const redirectUri = process.env.GSC_OAUTH_REDIRECT_URI || "https://smartpdfx.com/oauth2callback";
    ...
    const bodyHtml = `<!doctype html>
      <html>
        ...
        <body>
          ...
          ${refresh ? `<p><strong>Refresh Token:</strong></p><pre>${refresh}</pre>` : `<p><strong>No refresh token returned.</strong> Ensure <code>access_type=offline</code> and <code>prompt=consent</code> are set, then retry.</p>`}
          ${access ? `<p><em>Access token received (short-lived)</em></p>` : ""}
          <p>Next steps:</p>
          <ol>
            <li>Copy the refresh token above.</li>
            <li>Paste it into your <code>.env</code> as <code>GSC_REFRESH_TOKEN</code>.</li>
