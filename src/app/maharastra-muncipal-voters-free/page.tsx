...
export default function MaharashtraMunicipalVoterListPageFree() {
  ...
  // Server analysis: uses OCR-only route (free alternative to Genkit/LLM)
  const analyzePageOnServer = async (imageUri: string): Promise<Voter[]> => {
    const resp = await retryRequest(() => fetch('/api/voter-extract-free/analyze-page', {
      method: 'POST',
  ...
  const handleAnalyze = async () => {
    ...
    setTotalPages(0);
    try {
      // If the input is an image, analyze directly. If it's a PDF, process up to 50 pages.
  ...
  const handleDownloadExcel = () => {
    ...
    XLSX.writeFile(workbook, "maharashtra-municipal-voters-free.xlsx");
  };
  ...
  return (
    ...
    <main className="container mx-auto px-4 py-8 md:py-12">
      <header className="text-center">
        <h1 className="text-4xl font-bold font-headline">Maharashtra Muncipal Voter List tool (Free, OCR)</h1>
        <p className="text-lg text-muted-foreground mt-2">
          Extract structured voter data from municipal voter lists using OCR (no Genkit/LLM).
        </p>
      </header>
            ...
            {!result && !isAnalyzing && (
                 <div>
                    <div 
                        ...
                        onClick={() => document.getElementById('file-upload-municipal-free')?.click()}
                    >
                        <UploadCloud className="mx-auto h-12 w-12 text-primary" />
                        <p className="mt-4 font-semibold text-primary">Click to upload Image or PDF</p>
                        <Input 
                            id="file-upload-municipal-free"
                            type="file" 
                            className="hidden" 
            ...
            {isAnalyzing && (
                <div className="flex flex-col items-center text-center">
                    ...
                    {totalPages > 0 ? (
                      ...
                      <p className="text-muted-foreground">The OCR is reading the document. This may take a moment.</p>
                    )}
                </div>
            )}
