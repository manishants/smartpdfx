...
import { Edit, Eye, MoreHorizontal, Trash2, FileText } from 'lucide-react';
...
export default function DraftPostsPage() {
  ...
  const loadPosts = async () => {
    try {
      ...
      console.error('Error loading draft posts:', e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePublish = async (postId: string) => {
    try {
      ...
      if (!post) return;
      await cmsStore.updatePost(postId, { ...post, status: 'published' });
      await loadPosts();
    } catch (e) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">Manage posts saved as drafts</p>
        </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Drafts ({filtered.length})</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex mb-4">
            <Input placeholder="Search drafts..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
          </div>
