...
import { Key, Eye, EyeOff, Trash2, Lock } from "lucide-react";
...
export default function ApiKeySettingsPage() {
  ...
  const loadKeys = async () => {
    ...
    try {
      ...
      if (res.status === 401) {
        toast({ title: 'Unauthorized', description: 'Please login as superadmin', variant: 'destructive' });
  ...
  const toggleKeyEnabled = async (id: string, enabled: boolean) => {
    if (id === 'ENV') {
      toast({ title: 'Read-only key', description: 'Environment key cannot be toggled here.' });
      return;
    }
    try {
      const res = await fetch('/api/keys/gemini', {
  ...
  const handleDeleteKey = async (id: string) => {
    if (id === 'ENV') {
      toast({ title: 'Read-only key', description: 'Environment key cannot be deleted here.' });
      return;
    }
    try {
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Environment key is read-only; you can add additional keys below.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <Select value={rotationEnabled ? rotationStrategy : undefined as any} onValueChange={(v) => handleChangeStrategy(v as any)}>
                  ...
                  geminiKeys.map((k) => (
                    <TableRow key={k.id}>
                      <TableCell className="flex items-center gap-2">
                        {k.id === 'ENV' && <Lock className="h-4 w-4 text-muted-foreground" />}
                        {k.label || (k.id === 'ENV' ? 'Environment' : '-')}
                      </TableCell>
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-2">
                          <Button variant="outline" size="sm" onClick={() => toggleKeyEnabled(k.id, !k.enabled)} disabled={k.id === 'ENV'}>
                            {k.enabled ? (
                          ...
                          <Button variant="destructive" size="sm" onClick={() => handleDeleteKey(k.id)} disabled={k.id === 'ENV'}>
